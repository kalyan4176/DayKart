...
import { LayoutDashboard, Users, ShoppingBag, ShieldCheck, CheckCircle2, XCircle, User, Mail, Phone, AlertTriangle, Store, Plus, Trash2, Edit, FolderOpen, ClipboardList, RefreshCw, ChevronRight, Sliders, Tag, Gift, Percent, Calendar, Truck, Wallet, Search, ChevronDown, MessageSquare, Lock } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  // Mutation hooks
  const [sendPasswordOtpApi, { isLoading: isSendingOtp }] = useSendChangePasswordOtpMutation();
  ...
  const handleRequestOtp = async (e) => {
    e.preventDefault();
    if (!currentPassword) {
      showToast('Please enter your current password first.', 'error');
      return;
    }
    try {
      const res = await sendPasswordOtpApi().unwrap();
      showToast(res.message || 'OTP verification code sent to your registered email.', 'success');
      setIsOtpSent(true);
      setOtpCountDown(60);
    } catch (err) {
      showToast(err.data?.message || 'Failed to send OTP verification code.', 'error');
    }
  };

  const handleUpdatePassword = async (e) => {
    e.preventDefault();
    if (!currentPassword || !passwordOtp || !newPassword || !confirmNewPassword) {
    ...
    if (!passwordRegex.test(newPassword)) {
      showToast('Password must contain at least 8 characters with uppercase, lowercase, numbers, and symbols.', 'error');
      return;
    }
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const adminEmail = user?.email || 'daykart.services@gmail.com';
            ...
            {activeTab === 'profile' && (
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-sm space-y-6">
                  ...
                  {profileSuccess && (
                    <p className="text-xs text-emerald-500 font-bold bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/40 p-2.5 rounded-xl animate-fade-in">
                  ...
                  <form onSubmit={handleUpdateProfile} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      className="w-full bg-secondary hover:bg-cyan-600 text-white font-bold py-2.5 rounded-xl text-xs shadow-md transition cursor-pointer"
                    >
                      Update Profile
                    </button>
                  </form>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-sm space-y-6">
                  <h3 className="font-extrabold text-base text-slate-800 dark:text-slate-200 border-b border-slate-100 dark:border-slate-800 pb-3 flex items-center gap-2">
                    <ShieldCheck className="w-5 h-5 text-secondary" /> Change Password
                  </h3>
                  ...
                  <form className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative flex gap-3">
                        <div className="relative flex-1">
                          <input
                            ...
                            disabled={isOtpSent}
                            className="w-full bg-slate-100 dark:bg-slate-800 border border-transparent focus:border-secondary pl-10 pr-4 py-2.5 rounded-xl text-xs outline-none transition dark:text-slate-200 disabled:opacity-60"
                          />
                        ...
                        {!isOtpSent && (
                          <button
                            ...
                            className="bg-secondary hover:bg-cyan-600 disabled:bg-slate-200 dark:disabled:bg-slate-850 disabled:text-slate-400 text-white font-bold px-4 py-2.5 rounded-xl text-xs shadow-sm transition whitespace-nowrap active:scale-95 cursor-pointer flex items-center justify-center min-w-[130px]"
                          >
                            {isSendingOtp ? (
                      ...
                      <p className="text-[10px] text-slate-400 mt-1">
                        A 6-digit verification code will be sent to your registered admin email: <strong>{user?.email || 'daykart.services@gmail.com'}</strong>.
                      </p>
                    </div>

                    {/* Step 2: OTP and New Password Fields */}
                    {isOtpSent && (
                      <div className="space-y-4 pt-2 border-t border-dashed border-slate-100 dark:border-slate-800/80 animate-fade-in">
                        {/* OTP Verification Info */}
                        <div className="flex items-center justify-between bg-cyan-50 dark:bg-cyan-950/20 border border-cyan-100 dark:border-cyan-900/40 p-3 rounded-xl">
                          <div className="text-xxs font-bold text-cyan-700 dark:text-cyan-400 flex flex-col gap-0.5">
                            <span>Verification OTP sent! Check your inbox.</span>
                            <span className="text-[10px] font-medium text-slate-500">Need a new code?</span>
                          </div>
                          ...
                          <button
                            ...
                            className="text-xs text-secondary hover:underline disabled:text-slate-400 font-bold cursor-pointer"
                          >
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="w-full bg-slate-100 dark:bg-slate-800 border border-transparent focus:border-secondary pl-10 pr-4 py-2.5 rounded-xl text-xs outline-none transition font-mono tracking-widest text-slate-850 dark:text-slate-200"
                            />
                        ...
                        <div className="flex gap-3 pt-2">
                          ...
                          <button
                            ...
                            disabled={isChangingPassword || !passwordOtp || !newPassword || !confirmNewPassword}
                            className="flex-1 bg-secondary hover:bg-cyan-600 disabled:bg-slate-200 dark:disabled:bg-slate-800 text-white font-bold py-2.5 rounded-xl text-xs shadow-md transition flex items-center justify-center gap-1 cursor-pointer"
                          >
                            {isChangingPassword ? (
